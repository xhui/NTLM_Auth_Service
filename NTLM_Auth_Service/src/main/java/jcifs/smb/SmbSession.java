 ...
 */
    ...
    public static void loginValidation(UniAddress dc,
                        NtlmPasswordAuthentication auth ) throws SmbException
    {
    	SmbSession session= SmbTransport.getSmbTransport( dc, 0 ).getSmbSession( auth );
    	SmbTree tree = session.getSmbTree( LOGON_SHARE, null );
        if( LOGON_SHARE == null ) {
            tree.treeConnect( null, null );
        } else {
            Trans2FindFirst2 req = new Trans2FindFirst2( "\\", "*", SmbFile.ATTR_DIRECTORY );
        ...
        // TODO MX: After validation succeed, login off
